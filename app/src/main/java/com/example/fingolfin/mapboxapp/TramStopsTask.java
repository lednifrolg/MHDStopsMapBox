 ...
 */
public class TramStopsTask extends AsyncTask<MapView, Void, Boolean> {
    public static final String TAG = TramStopsTask.class.getSimpleName();

    private Context mContext;

    private MapView mMapView = null;
    private ArrayList<Marker> mTramMarkers = new ArrayList<>();

    public TramStopsTask(Context c) {
        mContext = c;
    }

    public void deleteMarkers() {
        for (Marker m : mTramMarkers) {
            m.remove();
        }
    }
    ...
    public void updateStops(JSONObject jObj) {
        ...
        Drawable drawable = ContextCompat.getDrawable(mContext, R.drawable.ic_directions_railway_black_18dp);
    ...
    private JSONObject getJson() {
        ...
        try {
            URL url = new URL(NetworkInfo.IP_ADDRESS + "/tram.php");
