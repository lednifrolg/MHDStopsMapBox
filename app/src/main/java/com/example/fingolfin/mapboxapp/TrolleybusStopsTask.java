 ...
 */
public class TrolleybusStopsTask extends AsyncTask<MapView, Void, Boolean> {

    public static final String TAG = TrolleybusStopsTask.class.getSimpleName();

    private Context mContext;

    private MapView mMapView = null;
    private ArrayList<Marker> mTrolleybusMarkers = new ArrayList<>();

    public TrolleybusStopsTask(Context c) {
        mContext = c;
    }

    public void deleteMarkers() {
        for (Marker m : mTrolleybusMarkers) {
            m.remove();
        }
    }
    ...
    public void updateStops(JSONObject jObj) {
        ...
        Drawable drawable = ContextCompat.getDrawable(mContext, R.drawable.ic_directions_subway_black_18dp);
    ...
    private JSONObject getJson() {
        ...
        try {
            URL url = new URL(NetworkInfo.IP_ADDRESS + "/trolley.php");
